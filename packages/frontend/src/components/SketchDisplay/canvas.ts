...
const drawPoint = (ctx: CanvasRenderingContext2D, point: DrawablePoint): void => {
...
const drawCircle = (ctx: CanvasRenderingContext2D, circle: DrawableCircle): void => {
	// Get the center of the canvas
	const centerX = ctx.canvas.width / 2
	const centerY = ctx.canvas.height / 2
...
const drawArc = (ctx: CanvasRenderingContext2D, arc: DrawableArc): void => {
	...
	drawArcWithPoints(ctx, { x: xs, y: ys }, { x: xe, y: ye }, { x: xc, y: yc }, arc.radius)
}
